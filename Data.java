...
public class Data {
  ...
  private String featureIsPresent = "T" ;
  ...
  public void load(String fileName) {
    ...
    String line, date, fields[] ;
    String[] patient ;
    for (int d=0 ; d<numberOfDays() ; d++) { data.add(new ArrayList<String[]>()) ; }
    try {
      fileReader = new FileReader(fileName) ;
      inputFile = new BufferedReader(fileReader) ;
      while ( (line=inputFile.readLine()) != null ) {
        fields = line.split(" ") ;
        if ( fields.length != features.numberOfFeatures()+1 ) { System.out.println( "ERROR: " + line ) ; }
        date = fields[0] ;
        patient = fields ;
        data.get(day(date)).add(patient) ;
      } // while
      inputFile.close() ;
    } catch (IOException e) {} ;
  }

  public int numberOfDays() { return dates.length ; }

  public int numberOfPatients(int day) { return data.get(day).size() ; }

  public int numberOfPatients(int day, int window) {
    int total = 0 ;
    for (int d=day ; d<day+window ; d++) { total += numberOfPatients(d) ; }
    return total ;
  }

  public boolean patientHasFeature(int day, int patient, int feature) {
    if ( data.get(day).get(patient)[feature+1].equals(featureIsPresent) ) return true ; else return false ;
  }

  public int numberOfPatientsWithFeature(int day, int feature) {
    int total = 0 ;
    for (int p=0 ; p<numberOfPatients(day) ; p++) { if ( patientHasFeature(day,p,feature) ) total++ ; }
    return total ;
  }

  public int numberOfPatientsWithFeature(int day, int window, int feature) {
    int total = 0 ;
    for (int d=day ; d<day+window ; d++) total += numberOfPatientsWithFeature(d,feature) ;
    return total ;
  }

  public double averageNumberOfPatients(int day, int window) {
    int total = 0 ;
    for (int d=day-(window/2) ; d<day+((window+1)/2) ; d++) {
      total += numberOfPatients(Math.min(Math.max(d,0),numberOfDays()-1)) ;
    }
    return (double)total/(double)window ;
  }

  public int day(String date) {
    for (int d=0 ; d<dates.length ; d++) { if ( dates[d].equals(date) ) return d ; }
    return -1 ;
  }

  public String date(int day) { return dates[day] ; }
